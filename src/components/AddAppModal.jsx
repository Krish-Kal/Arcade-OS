// AddAppModal - Premium unified UI version
import React, { useState } from 'react'
import { X, FolderOpen, Grid3X3, ImagePlus } from 'lucide-react'
import { useStore } from '../store/useStore'

const isElectron = typeof window !== 'undefined' && window.arcadeOS

const CATEGORIES = [
  'Dev Tools', 'Communication', 'Streaming', 'Media',
  'Productivity', 'Browser', 'Utilities', 'Design',
  'Office', 'Security', 'Other'
]
...
export default function AddAppModal({ onClose }) {
  ...
  const browsePath = async () => {
    if (!isElectron) {
      update('path', 'C:\\Program Files\\App\\app.exe')
      return
    }
  ...
  // ✅ FIX 1: Added missing browseImage function
  const browseImage = () => {
    ...
    input.onchange = (e) => {
      ...
      reader.onload = () => update('icon', reader.result)
      ...
      >
        ...
        <div style={{
          ...
          <Field label="Category">
            ...
            >
              {CATEGORIES.map(c => (
                <option key={c} value={c} style={{ background: '#0f0f15' }}>
                  {c}
                </option>
              ))}
            </select>
          ...
          <Field label="Game Cover (optional)">
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <IconButton onClick={browseImage}>
                {/* ✅ FIX 2: replaced missing Upload icon */}
                <ImagePlus size={14} />
              </IconButton>
            </div>
